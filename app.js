...
function renderMaster() {
  ...
  /* --- day by day, Mastersheet layout: 3-tier header --- */
  const { days, year, month } = monthBounds(m);
  ...
  const tier1 = `<tr class="group-head">
    <th rowspan="3">Date</th><th rowspan="3" class="num">Total</th>
    <th colspan="3" rowspan="2">Reachouts by Type</th>
    <th colspan="9">Status</th>
    <th colspan="6">If Interested</th>
    <th colspan="6">Next Stage</th>
    <th colspan="2" rowspan="2">Meeting Split</th></tr>`;
  const tier2 = `<tr class="group-head">
    ${SEGMENTS.map((sg) => `<th colspan="3">${esc(sg)}</th>`).join("")}
    ${SEGMENTS.map((sg) => `<th colspan="2">${esc(sg)}</th>`).join("")}
    ${SEGMENTS.map((sg) => `<th colspan="2">${esc(sg)}</th>`).join("")}</tr>`;
  const tier3 = `<tr>
    ${SEGMENTS.map((sg) => `<th class="num">${esc(sg)}</th>`).join("")}
    ${SEGMENTS.map((sg) => RESULTS.map((r) => `<th class="num">${esc(r)}</th>`).join("")).join("")}
    ${SEGMENTS.map((sg) => shareLabel(sg).map((l) => `<th class="num">${l}</th>`).join("")).join("")}
    ${SEGMENTS.map((sg) => doneLabel(sg).map((l) => `<th class="num">${l}</th>`).join("")).join("")}
  ...
  $("tbl-daily").innerHTML = `<thead>${tier1}${tier2}${tier3}</thead><tbody>${body}</tbody>`;
