...
const state = { user: null, profile: null, meetings: [], team: [], plans: [], contacts: [], leads: [], myLeads: [], page: null, editingId: null };
...
const canLogHere = () => state.profile && ["rm", "admin", "superadmin"].includes(state.profile.role);
...
// Contacts registry: RM sees only what they created; everyone else
// (including Observer) sees all — less sensitive than the leads below.
async function loadContacts() {
  const col = collection(db, "contacts");
  const q = canViewAll() ? col : query(col, where("createdByEmail", "==", state.user.email));
  const snap = await getDocs(q);
  state.contacts = snap.docs.map((d) => ({ id: d.id, ...d.data() }))
    .sort((a, b) => (a.name || "").localeCompare(b.name || ""));
}

// Leads: RM sees only their own. Admin/Superadmin see all. Observer gets
// neither branch here — state.myLeads stays empty and state.leads is
// never fetched for them, on top of the rules blocking it anyway.
...
function toggleContactMode() {
  ...
  if (on) $("f-contactIdPrefix").textContent = `${contactPrefix()}-`;
  if (!on) {
  ...
  if (!$("f-contactMode").value) $("f-contactMode").value = "New";
  applyContactMode();
}
...
async function lookupContact() {
  ...
  $("f-prospectName").value = ""; $("f-phone").value = ""; $("f-email").value = ""; $("f-address").value = "";
  status.hidden = false;
  if (digits.length !== 10) { status.textContent = "Enter the contact's 10-digit mobile number."; return; }
...
$("f-contactMode").addEventListener("change", applyContactMode);
...
function renderLeadsList() {
    ...
    wrap.innerHTML = currentLeads.map((l, i) => `
      <div class="lead-row">
        ...
        <input type="date" max="${todayISO()}"
          title="${l.status === "Meeting Done" ? "Date the meeting happened" : "Expected meeting date"}"
  ...
  wrap.querySelectorAll("[data-lead-status]").forEach((el) =>
    el.addEventListener("change", () => { currentLeads[+el.dataset.leadStatus].status = el.value; renderLeadsList(); }));
  ...
  for (const r of rows) {
    ...
    const leads = rLeads.length
      ? `<div class="entry-leads">${rLeads.map((l) =>
          `<span class="tag tag-flat">${esc(l.leadName)} · ${esc(l.leadPhone)} · ${esc(l.status)}${l.leadDate ? " · " + esc(l.leadDate) : ""}</span>`).join(" ")}</div>`
      : "";
...
function startEdit(id) {
  ...
  $("f-contactId").value = r.contactId ? r.contactId.replace(/^[A-Z]+-/, "") : "";
  $("contact-lookup-status").hidden = true;
...
function renderHistory(mine) {
  ...
  $("tbl-history").innerHTML = `<thead><tr>
    ...
    <th>Lead / docs</th><th>Next stage</th><th>Follow-up</th><th>Remarks</th>
    </tr></thead><tbody>${
      rows.length ? rows.map((r) => `<tr>
        ...
        <td>${esc(r.source || "—")}</td><td>${resultTag(r.result)}</td>
        <td>${esc(r.shared || "—")}</td><td>${esc(r.progressed || "—")}</td>
        <td class="num">${esc(r.followUpDate || "")}</td><td class="wrap">${esc(r.remarks || "")}</td></tr>`).join("")
...
function renderMaster() {
  ...
  /* --- leads shared with the team (Admin/Superadmin only — never Observer) --- */
  $("leads-card").hidden = !canSeeAllLeads();
  if (canSeeAllLeads()) {
    const leadRows = state.leads.filter((l) => l.date >= from && l.date <= to)
      .sort((a, b) => (b.date || "").localeCompare(a.date || ""));
...
function exportLeads() {
  if (typeof XLSX === "undefined") { toast("Excel library didn't load. Check your connection."); return; }
  const { from, to, label } = exportRange();
  const rows = state.leads.filter((l) => l.date >= from && l.date <= to);
...
function renderContacts() {
  ...
  const rows = [...state.contacts].sort((a, b) => (a.name || "").localeCompare(b.name || ""));
  ...
  $("tbl-contacts").innerHTML = `<thead><tr>
    <th>ID</th><th>Name</th><th>Segment</th><th>Phone</th><th>Email</th><th class="wrap">Address</th>
    ${ownerCol}${actionCol}
    </tr></thead><tbody>${
      rows.length ? rows.map((c) => `<tr>
        ...
        ${isSuperAdmin() ? `<td><button class="btn-link" data-transfer="${esc(c.id)}">Transfer</button></td>` : ""}
        </tr>`).join("")
        : `<tr><td colspan="${mine ? 6 : (isSuperAdmin() ? 9 : 8)}" class="empty">Nothing on file yet.</td></tr>`
    }</tbody>`;

  $("tbl-contacts").querySelectorAll("[data-transfer]").forEach((b) =>
    b.addEventListener("click", () => transferContact(b.dataset.transfer)));
}

async function transferContact(contactId) {
  const c = state.contacts.find((x) => x.id === contactId);
  if (!c) return;
  const code = (prompt(`Transfer ${c.name} (${contactId}) to which employee ID?`, "") || "").trim();
  if (!code) return;
  const target = state.team.find((u) => (u.employeeId || "").trim() === code);
  if (!target) { toast(`No team member with employee ID "${code}".`); return; }
  if (target.email === c.createdByEmail) { toast(`${c.name} is already owned by ${target.name}.`); return; }
  if (!confirm(`Move ${c.name} from ${c.createdByName || c.createdByEmail} to ${target.name}?`)) return;
  try {
    await updateDoc(doc(db, "contacts", contactId), {
      createdByEmail: target.email, createdByEmployeeId: target.employeeId || "", createdByName: target.name || target.email
    });
    toast(`${c.name} transferred to ${target.name}`);
    await loadContacts();
    renderContacts();
  } catch (e) {
    toast("Couldn't transfer that. " + (e.message || ""));
  }
}
