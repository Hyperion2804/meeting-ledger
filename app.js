...
const canViewAll = () => state.profile && (state.profile.role === "admin" || state.profile.role === "founder");
const roleLabel = (r) => r === "admin" ? "Admin" : r === "founder" ? "Founder" : "RM";
    ...
    <th>Name</th><th>Email</th><th>Role</th><th>Status</th><th class="num">Meetings logged</th><th></th>
      ...
      state.team.length ? state.team.map((u) => {
        ...
        return `<tr>
          ...
          <td><span class="tag ${u.role === "admin" ? "tag-green" : u.role === "founder" ? "tag-amber" : "tag-flat"}">${roleLabel(u.role)}</span></td>
