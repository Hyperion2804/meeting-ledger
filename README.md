...
In the left menu open Build → Firestore Database → Create database.
Pick Production mode and a region near you (`asia-south1` for India).
In the left menu open Build → Authentication → Get started.
...
You'll see a `firebaseConfig = { ... }` block. Copy it.
...
Open `firestore.rules` and change one line near the top:
```
...
In Firebase go to Firestore Database → Rules, delete what's there,
...
entry by hand, once.
Firebase → Firestore Database → Data → Start collection.
Collection ID: `users`
Document ID: your work email, all lowercase — e.g. `you@yourcompany.com`
Add six fields:
Field	Type	Value
...
`active`	boolean	`true`
Save. From now on you add everyone else from the Team tab inside the
...
Create a new repository on GitHub. Public is fine.
Upload every file in this folder to the root of the repo
...
Wait a minute. Your app is live at
...
Editing lives on My History, not the Log page. The compact list under
"Your month" is now view-only — click into My history to correct a
past entry. There's no Delete button anywhere for RMs; a mistake gets
corrected by editing it, not erased.
Contacts has a second sub-tab: Investors. Unlike Wealth Managers and
Channel Partners, Investors don't get an ID or a New/Existing flow — this
tab is just a live, deduplicated list built from meetings already logged,
one row per person, most recent meeting's details shown. Same visibility
as everywhere else: your own, your reports' if you're a Team Lead, everyone
else's if you're Admin/Superadmin/Observer.
A WM/CP meeting defaults sensibly. First meeting defaults to New,
Follow-up defaults to Existing — since a follow-up almost always means you
already know this contact. Either way it's just a starting point; pick the
other one any time, and once you do, changing the meeting type afterward
