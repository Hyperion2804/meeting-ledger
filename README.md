...
**Master** — the funnel, a per-RM table, a Plan vs Achievement table, and a
day-by-day grid matching your Mastersheet layout. **Download Excel** gives you
a workbook with three tabs: `Master` (daily totals across everyone), `Data`
(one row per person per day, in the exact column order as your Mastersheet's
`Data` tab — including Designation and Employee ID), and `All Meetings`
(every row, including the reference name and not-interested reason, but
never the leads). Pick a month, or fill in the two custom-range date fields
next to it to export any exact range instead.
...
**My meetings** — log your own meetings, and set tomorrow's plan from the
card underneath the form.

**Team** — Super Admin can add people, change anyone's role (RM, Admin, or
Super Admin), and remove or restore access. Removing access keeps their
history; it just stops them signing in. A plain Admin can see the same table
but everything is read-only — no add form, no role dropdowns, no remove
buttons.
...
**Custom date ranges.** Next to the month picker on Master, two date fields
let you pick an exact range instead — fill both and **Download Excel** (or
**Download leads**) uses that range instead of the selected month.
